...
namespace TypeMake.Cpp
{
    public class AndroidBatchProjectGenerator
    {
        ...
        private IEnumerable<String> GenerateLines(String BuildBatchPath, String BaseDirPath)
        {
            ...
            var JarDirs = conf.Options.ContainsKey("gradle.jarDirs") ? conf.Options["gradle.jarDirs"].Split(';').Select(d => d.AsPath().RelativeTo(BaseDirPath)).ToList() : new List<PathString> { };
            var JarFiles = JarDirs.SelectMany(d => System.IO.Directory.GetFiles(d, "*.jar", System.IO.SearchOption.AllDirectories).Select(f => f.AsPath())).ToList();
