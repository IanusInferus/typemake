...
namespace TypeMake
{
    public static class VariableCollection
    {
        public static KeyValuePair<Variables, List<VariableItem>> GetVariableItems()
        {
            ...
            l.Add(new VariableItem
            {
                VariableName = nameof(Variables.EnableLlvmlibcOverlay),
                DependentVariableNames = new List<String> { nameof(Variables.CLibrary) },
                GetVariableSpec = () =>
                {
                    if ((Variables.CLibrary == Cpp.CLibraryType.glibc) || (Variables.CLibrary == Cpp.CLibraryType.musl))
                    {
                        return VariableSpec.CreateBoolean(new BooleanSpec
                        {
                            DefaultValue = false
                ...
                SetVariableValue = v => Variables.EnableLlvmlibcOverlay = v.Boolean
            });
                        ...
                        if (Variables.HostOperatingSystem == Cpp.OperatingSystemType.Windows)
                        {
                            ((Action)(() =>
                            {
                                foreach (var ProgramFiles in new String[] { Utils.CheckedGetEnvironmentVariable("ProgramFiles"), Utils.CheckedGetEnvironmentVariable("ProgramFiles(x86)") })
                                {
                                    if (ProgramFiles != "")
                                    {
                                        foreach (var Version in new int[] { 18, 2022 })
                                        {
            ...
            l.Add(new VariableItem
            {
                ...
                GetVariableSpec = () =>
                {
                    if (Variables.Toolchain == Cpp.ToolchainType.VisualStudio)
                    {
                        var VSVersion = Variables.VSDir.ToString().Contains("18") ? 18 : 2022;
