...
namespace TypeMake
{
    public static class BuildScript
    {
        public static void GenerateRetypemakeScript(Cpp.OperatingSystemType HostOperatingSystem, PathString SourceDirectory, PathString BuildDirectory, Shell.EnvironmentVariableMemory Memory, bool OverwriteRetypemakeScript)
        {
            if (HostOperatingSystem == Cpp.OperatingSystemType.Windows)
            {
                ...
                }
                {
                    ...
                    foreach (var ProgramFiles in new[] { Utils.CheckedGetEnvironmentVariable("ProgramFiles"), Utils.CheckedGetEnvironmentVariable("ProgramFiles(x86)") })
                    {
            ...
            else
            {
                ...
                if (OverwriteRetypemakeScript || !File.Exists(RetypemakePath))
                {
                    ...
                    Utils.CheckedShellExecute("chmod", "+x", RetypemakePath);
                }
                else
                {
                    WriteLineError("Retypemake script exists, script generation skipped.");
        ...
        public static void GenerateBuildScriptUnix(String TargetOperatingSystemDistribution, Cpp.ToolchainType Toolchain, Cpp.OperatingSystemType HostOperatingSystem, PathString BuildDirectory, Cpp.ConfigurationType Configuration, int MaxProcessCount, PathString Ninja, bool ForceRegenerate)
        {
            if (Toolchain == Cpp.ToolchainType.Ninja)
            {
                ...
                else
                {
                    ...
                    Utils.CheckedShellExecute("chmod", "+x", BuildPath);
                }
            }
        }
        public static void GenerateBuildScriptXCode(Cpp.OperatingSystemType HostOperatingSystem, PathString BuildDirectory, Cpp.ConfigurationType Configuration, int MaxProcessCount, PathString XCodeDir, List<String> CppSortedProjectNames, bool ForceRegenerate)
        {
            ...
            {
                Lines.Add($"{Shell.EscapeArgumentForShell(XCodeDir / "Contents/Developer/usr/bin/xcodebuild", Shell.ShellArgumentStyle.Bash)} -project projects/{CppSortedProjectName}.xcodeproj -configuration {Configuration} -jobs {MaxProcessCount} -allowProvisioningUpdates");
            }
            Lines.Add("");
            var BuildPath = BuildDirectory / "build.sh";
            TextFile.WriteToFile(BuildPath, String.Join("\n", Lines), new System.Text.UTF8Encoding(false), !ForceRegenerate);
